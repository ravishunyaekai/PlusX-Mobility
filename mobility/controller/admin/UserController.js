...
export const riderDetails = async (req, resp) => {
  ...
  if (!riderId)
    return resp
      .status(200)
      .json({ status: 0, code: 400, message: ["Rider ID is required"] });
...
export const deleteRider = async (req, resp) => {
  ...
  if (!rider_id)
    return resp.json({ status: 0, code: 422, message: "Rider ID is required" });

  try {
    const db = callDatabase(req.db);

    const [[rider]] = await db.execute(
      "SELECT profile_img, rider_name, last_name, rider_email, country_code, rider_mobile, emirates, area, country, date_of_birth, added_from FROM riders WHERE rider_id = ?",
      [rider_id],
    );
    if (rider.length === 0)
...
export const addRefundAmount = asyncHandler(async (req, resp) => {
  ...
  await pushNotification(
    rider.fcm_token,
    template.heading,
    template.desc({ amount }),
    "RDRFCM",
    template.href({ riderId }),
  );
...
export const bookngIncompleteByadmin = asyncHandler(async (req, resp) => {
  ...
  const result = await incompletecompleteride(
    booking_id,
    station_id,
    "manual",
    lock_number,
...
const incompletecompleteride = async (
  booking_id,
  station_id,
  handover_type,
  lock_number,
  ...
  try {
    ...
    if (!bookingDetail)
      return {
        ...
        code: 422,
        message:
          "Ride completion failed. Booking not found or rider account unavailable.",
      };
