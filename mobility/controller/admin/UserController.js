    ...
    try {
        const bookingDetail = await queryDB(`
            SELECT
                ...
                cb.rider_id, r.amount as wallet_balance, cb.status, cb.price, msl.state_id
        ...
        await db.execute(` UPDATE riders SET out_standing_cost = ? WHERE rider_id = ?`, [ parseFloat( final_amount ), rider_id] );
        ...
        const description =  typeof db_logs_data.description === "string" ? JSON.parse(db_logs_data.description) : db_logs_data.description;
...
export const bookngIncompleteByadmin = asyncHandler(async(req,resp)=>{
    ...
    return resp.json({message: result.message, status: result.status, code: result.code, result : result});
})
...
const incompletecompleteride = async (booking_id, station_id, handover_type, lock_number, end_time, comment) => {
    try {
        ...
        const pickTimeDB = moment( pickMoment, "YYYY-MM-DD HH:mm:ss" );
        const endTimeN   = moment( end_time, "hh:mm A" );

        // merge date from pickTime + time from endTime
        const finalEndTime  = moment( `${pickTimeDB.format("YYYY-MM-DD")} ${endTimeN.format("HH:mm:ss")}`, "YYYY-MM-DD HH:mm:ss" );
        if (finalEndTime.isBefore(pickMoment)) {
            return { status: 0, code: 422, message: "End time cannot be earlier than pick time." };
        }
        let diffInMinutes = finalEndTime.diff(pickTimeDB, "minutes");
        const diffInSeconds = finalEndTime.diff(pickTimeDB, "seconds");
        ...
        const bookingParams = {
            ...
            price           : final_amount,
            time_taken      : diffInMinutes,
            drop_time       : finalEndTime.format("YYYY-MM-DD HH:mm:ss"),
            handover_type   : handover_type,
            hand_over_station: station.station_name,
            total_time      : total_taken_time,
            lock_number     : lock_number
        ...
        emailQueue.addEmail(
            ...
            mail_template.content({
                ...
                cycle_id   : bookingDetail.cycle_id, 
                pick_time  : moment(bookingDetail.pick_time).format('hh:mm A'),
                drop_time  : finalEndTime.format("hh:mm A"),
                time_taken : diffInMinutes,
                amount     : final_amount
            })
        );
