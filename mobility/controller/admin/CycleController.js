...
export const addCycle = asyncHandler(async (req, resp) => {
  ...
  if (qr_code == "undefined") {
    return resp.json({
      status: 0,
      code: 422,
      message: "qr code can not be blank",
    });
  }
  ...
  const shop_gallery =
    uploadedFiles["shop_gallery"]?.map((file) => file.filename) || [];
  let row_select;
  switch (cycle_type) {
    case "ecycle":
      row_select = "no_ecyle";
    ...
    case "cycle":
      ...
      break;
    default:
      return resp.json({ status: 0, code: 400, message: "Invalid cycle type" });
  }
  ...
  if (Number(cycle_count) >= Number(cycle_limit)) {
    return resp.json({
      status: 0,
      code: 400,
      message: "Cannot add more cycles. Station limit reached!",
    });
  }
  try {
    ...
    if (check_cycle.length > 0) {
      return resp.json({
        status: 0,
        code: 400,
        message: "This Cycle ID or QR Code already exists.",
      });
    }
    ...
    const [existingPrice] = await conn.execute(
      `SELECT id FROM cycle_pricing WHERE station_id = ? AND type_of_cycle = ? LIMIT 1`,
      [station_id, cycle_type],
    );

    if (existingPrice.length === 0) {
      const [insert_cycle_price] = await conn.execute(
        `INSERT INTO cycle_pricing (station_id, type_of_cycle, base_price, created_by) VALUES (?, ?, ?, ?)`,
        [station_id, cycle_type, base_price, userId],
      );
      if (insert_cycle_price.affectedRows === 0) {
        await conn.rollback();
        return resp.json({
          status: 0,
          message: "Failed to add  Cycle! Please try again after some time.",
        });
      }
    } else {
      const [updated_cycle_price] = await conn.execute(
        `UPDATE cycle_pricing set base_price=? where station_id=? and type_of_cycle = ? `,
        [base_price, station_id, cycle_type],
      );
    ...
    if (shop_gallery.length > 0) {
      const values = shop_gallery.map((filename) => [
        station_id,
        cycle_id,
        cycle_type,
        filename,
      ...
      await conn.execute(
        `INSERT INTO cycle_gallery (station_id,cycle_id,cycle_type, image) VALUES ${placeholders}`,
        values.flat(),
      );
    }
    await conn.commit();
...
export const editCycle = asyncHandler(async (req, resp) => {
  ...
  try {
    ...
    const [cycle_list] = await conn.execute(
      `SELECT * FROM cycle_list where cycle_id=? limit 1`,
      [cycle_id],
    );
    ...
    const [cycle_backup] = await conn.execute(
      ...
      [
        ...
        "admin",
        userId,
        JSON.stringify(cycle_list[0]),
        cycle_id,
      ],
    );
    if (cycle_backup.affectedRows === 0)
      throw new Error(" log for cycle_list insert failed!");
    ...
    if (price_backup.affectedRows === 0)
      throw new Error("Log for cycle_pricing insert failed!");
    ...
    if (station_id !== cycle_list[0].station_id)
      updates_params.lock_number = "";
    ...
    const [[check_pricing]] = await conn.execute(
      `
            SELECT id 
            from cycle_pricing 
            where station_id = ? and type_of_cycle = ? `,
      [station_id, cycle_type],
    );
    if (check_pricing) {
      await conn.execute(
      ...
      await conn.execute(
        `INSERT INTO cycle_pricing(base_price,station_id,type_of_cycle)  values(?,?,?) `,
        [base_price, station_id, cycle_type],
      );
    }
    if (cycle_gallery.length > 0) {
      const values = cycle_gallery.map((filename) => [
        station_id,
        cycle_id,
        cycle_type,
        filename,
...
export const deletefromTable = asyncHandler(async (req, resp) => {
  try {
    ...
    if (!delelted)
      resp.json({ status: 0, code: 400, message: "Image was not deleted!" });
...
export const cycleList = asyncHandler(async (req, resp) => {
  ...
  if (start_date && end_date) {
    ...
    const startFormattedDate = `${startToday.getFullYear()}-${(
    ...
    const modifiedStartDateTime = moment(givenStartDateTime).subtract(
      4,
      "hours",
    ); // Subtract 4 hours
    ...
    const formattedEndDate = `${endToday.getFullYear()}-${(
      endToday.getMonth() + 1
    )
      .toString()
...
export const cycleDelete = asyncHandler(async (req, resp) => {
  ...
  const [cycleRows] = await db.execute(
    "SELECT * FROM cycle_list where cycle_id=?",
    [cycle_id],
  );
  ...
  await db.execute(
    `INSERT INTO db_logs (table_name, action_type, user_type, created_by, changed_data) 
    VALUES (?, ?, ?, ?, ?)`,
    ["Deleted Cycle", "deleted", "admin", userId, JSON.stringify(cycleData)],
  );

  const delte_cycle = await db.execute(
    `DELETE FROM cycle_list WHERE cycle_id = ?`,
    [cycle_id],
  );
  const deltegallery = await db.execute(
    `DELETE FROM cycle_gallery WHERE cycle_id = ?`,
    [cycle_id],
  );
  ...
  if (delte_cycle && deltegallery) {
    resp.json({
      status: 1,
      code: 200,
      message: "This Cycle Deleted Successfully!",
    });
  }
});
...
export const qrCode = asyncHandler(async (req, resp) => {
  ...
  const [check_cycle] = await db.execute(
    `SELECT id FROM cycle_list where  cycle_id=? `,
    [cycle_id],
  );
  ...
  return resp.json({
    ...
    code: 200,
    message: ["success"],
    data: { qrcode: forQr_code, qr_image },
  });
});
...
export const cycledetails = asyncHandler(async (req, resp) => {
  ...
  if (!cycle)
    return resp
      .status(404)
      .json({ status: 0, code: 404, message: "Cycle not found." });
  ...
  try {
    ...
    switch (true) {
      ...
      default:
        return resp.json({
          status: 0,
          message: "city_id or country_id is required",
        });
    }
    ...
    if (scheduleFilters.start_date && scheduleFilters.end_date) {
      ...
      const schEnd = moment(scheduleFilters.end_date, "YYYY-MM-DD").format(
        "YYYY-MM-DD",
      );
    ...
    return resp.json({
      status: 0,
      message: "Error fetching cycle booking lists",
    });
  }
};
...
export const FaildcycleBookingList = async (req, resp) => {
  try {
    ...
    return resp.json({
      status: 0,
      message: "Error fetching failed booking list",
    });
  }
};
...
export const cycleInvoiceDetails = asyncHandler(async (req, resp) => {
  ...
  try {
    ...
    cycle_booking.taxPrice =
      total_cost.toFixed(2) == cycle_booking.price.toFixed(2)
        ? 0
        : (total_cost * 0.18).toFixed(2);
  ...
  try {
    ...
    if (!cycleBooking)
      return resp.json({
        ...
        code: 400,
        message: [
          "Comments cannot be added for this booking ID, as the issue appears to have been resolved.",
        ],
      });
  ...
  try {
    ...
    if (!cycleBooking)
      return resp.json({
        ...
        code: 400,
        message: [
          "Comments cannot be change status of this booking ID, as the issue appears to have been resolved.",
        ],
      });
...
export const cycleOnOff = asyncHandler(async (req, resp) => {
  ...
  if (!isValid) return resp.json({ status: 0, code: 422, message: errors });
  const cycle_state = device_status;
  ...
  if (!cycleData)
    return resp.json({
      status: 0,
      code: 201,
      message: ["The Cycle ID is incorrect!"],
    });

  // await updateRecord('cycle_list', { device_status }, [ 'cycle_id' ], [ cycle_id ] );
  await updateRecord("cycle_list", { cycle_state }, ["cycle_id"], [cycle_id]);

  // const payload = (device_status == 1 ) ? `ON,${cycle_id}` : `OFF,${cycle_id}`;
  const payload = cycle_state == 1 ? `ON,${cycle_id}` : `OFF,${cycle_id}`;
  client.publish(`/supro/GW/${cycleData.gateway_id}/UP`, payload, {
    qos: 0,
    retain: false,
  });

  // const msgT = (device_status == 1 ) ? `unlocked` : `locked`;
  const msgT = cycle_state == 1 ? `unlocked` : `locked`;
  return resp.json({
    status: 1,
    code: 200,
    message: `Cycle ${msgT} successfully!`,
  });
});
...
export const lockerOpen = asyncHandler(async (req, resp) => {
  ...
  if (!locker_data)
    return resp.json({
      status: 0,
      code: 201,
      message: ["The Lock no. is incorrect!"],
    });
...
export const RefundRequestList = async (req, resp) => {
  try {
    ...
    const params = {
      ...
      columns: `id, rider_id, booking_id, user_name, contact_no, requested_amount,refund_amount, deduction_amount, refund_amount, status, 
                ${formatDateTimeInQuery(["created_at"])} `,
      sortColumn: "created_at DESC",
      sortOrder: "",
      page_no,
...
export const approveRefundRequestOld = asyncHandler(async (req, resp) => {
  try {
