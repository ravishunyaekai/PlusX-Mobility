...
export const startScanCycleQr = asyncHandler(async (req, resp) => {
    ...
    if (currTime < pricing_details.open_time || currTime > pricing_details.close_time) {
        ...
        // const msg =  `The station is currently closed. You can't book a ride at the moment. Please book between 10 AM and 7 PM.`;
    ...
    // if ( pricing_details.battery_health < 1 ) return resp.json({ status: 0, code: 201, error_type:'battery', message: ['This cycle has low battery. Please try another cycle.'] });
...
export const completeLockerQr = asyncHandler(async (req, resp) => {
    ...
    await db.execute(`
        ...
        WHERE rider_id = ?`, [ parseFloat(final_amount), rider_id]
    );
    const bookingParams = { 
        status          : "CMP",
        ...
        hand_over_station: cycle_lock.dropoff_station
    }
    const update_booking = await updateRecord('cycle_booking', bookingParams, ['booking_id'], [booking_id] );
    ...
    if(db_logs_data?.description){
        ...
        ? JSON.parse(db_logs_data.description) : db_logs_data.description;
...
const completeride = async (rider_id, booking_id, station_id, handover_type, lock_number ) => {
    try {
        ...
        const base_duration  = Number(bookingDetail.base_duration); /// 10
        const base_price     = parseFloat(bookingDetail.per_min_cost);  // 1 
        const post_price     = parseFloat(bookingDetail.post_price);
        const wallet_balance = parseFloat(bookingDetail.wallet_balance)
        ...
        final_amount : finalAmountNum, booking_id: booking_id };    
    } catch(err) {
        console.log(err);
