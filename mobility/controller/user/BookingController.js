...
import { webHooktryCatchErrorHandler } from "../../../common/controller/webhookController.js";

export const validateRideStart = async ({
    rider_id,
    cycle_id,
    queryDB,
    db,
    ...
    if (!rider) {
        return {
            ...
            response: {
                ...
                code: 201,
                message: ["Rider not found. Please log in again or contact support."],
            },
        };
    }

    // Cycle
    const cycle = await queryDB(`SELECT * FROM cycle_list WHERE cycle_id = ?`, [
        cycle_id,
    ]);
    ...
    if (!cycle) {
        return {
            ...
            response: {
                status: 0,
                error_type: "Cycle-Not-Found",
                code: 201,
                message: ["Cycle not found. Please log in again or contact support."],
            },
        };
    }
    ...
    JOIN country coun
        ON coun.country_id = cl.country_id
    JOIN cycle_pricing cp
        ON cp.type_of_cycle = cl.cycle_type
       AND cp.station_id = cl.station_id
    WHERE cl.cycle_id = ?
      AND cl.lock_number != ''
    LIMIT 1
    `,
    ...
    if (!pricing) {
        return {
            ...
            response: {
                status: 0,
                error_type: "invaild-QR",
                code: 201,
                message: [
                    "The cycle QR code is incorrect. Please scan the correct QR code.",
                ],
            },
        };
    }
    ...
    console.log(
        "\npricing",
        Number(pricing.security_deposit),
        Number(pricing.min_sec_deposit),
    );
    // Security Deposit
    if (Number(pricing.security_deposit) < Number(pricing.min_sec_deposit)) {
        return {
            success: false,
            response: {
                status: 0,
                ...
                code: 201,
                message: [
                    "Your security deposit is below the required amount. Please top up your security deposit to continue.",
                ],
            },
        };
    }
    ...
    // Wallet
    if (Number(pricing.wallet_balance) < Number(pricing.min_price)) {
        return {
            success: false,
            response: {
                status: 0,
                ...
                code: 201,
                message: [
                    "Your wallet balance is below the required amount. Please top up your security deposit to continue.",
                ],
            },
        };
    }
    ...
    // Outstanding
    if (Math.abs(Number(pricing.out_standing_cost)) > 0) {
        return {
            success: false,
            response: {
                status: 0,
                error_type: "balance",
                code: 201,
                message: ["Please clear your pending balance to start a new ride."],
            },
        };
    }
    ...
    if (currTime < pricing.open_time || currTime > pricing.close_time) {
      ...
      AND module_name = ?
      `,
            ["slot-error", "mobility-slot-booking"],
        );

        errContent.content = errContent.content
            .replace("10 AM", formatTime(pricing.open_time))
            .replace("7 PM", formatTime(pricing.close_time));

        return {
            success: false,
            response: {
                status: 0,
                error_type: "invalid slot",
                code: 201,
...
export const startScanCycleQrOld = asyncHandler(async (req, resp) => {
    ...
    const rider_details = await queryDB(
        `
            SELECT *
            FROM riders
            WHERE rider_id = ?
    ...
    const cycle_details = await queryDB(
        `
            SELECT *
            FROM cycle_list
            WHERE cycle_id = ?
    ...
    const pricing_details = await queryDB(
        ...
        SELECT 
            ...
            msl.station_name as pickup_station, 
            msl.address, 
            cl.cycle_type, 
            cl.id, 
            cl.station_id, 
        ...
        JOIN cities ct 
        ON cl.city_id = ct.city_id 
        JOIN cycle_pricing cp 
        ON cp.type_of_cycle = cl.cycle_type 
        and cl.station_id=cp.station_id
        WHERE cl.cycle_id = ? and cl.lock_number != '' limit 1 `,
        [rider_id, qrcode],
    );
    ...
    if (pricing_details.security_deposit < 100.0) {
        return resp.json({
            status: 0,
            error_type: "balance",
            code: 201,
    ...
    if (pricing_details.wallet_balance < pricing_details.min_price) {
        return resp.json({
            status: 0,
            error_type: "balance",
            code: 201,
    ...
    if (parseFloat(pricing_details.out_standing_cost) > 0) {
        return resp.json({
            status: 0,
            code: 201,
            error_type: "balance",
    ...
    if (
        ...
        currTime > pricing_details.close_time
    ) {
        const formatTime = (time) => {
            const [hour, minute] = time.split(":");
            let h = parseInt(hour);
        ...
        errContent.content = errContent.content
            .replace("10 AM", formatTime(open_time))
            .replace("7 PM", formatTime(close_time));
    ...
    return resp.json({
        status: 1,
        code: 200,
        cycle_id: pricing_details.cycle_id,
        message: ["Cycle verified successfully."],
    });
});
...
export const startScanCycleQr = asyncHandler(async (req, resp) => {
    try {
        ...
        // Common validations
        const validation = await validateRideStart({
            rider_id,
            cycle_id: qrcode,
            queryDB,
            db,
        ...
        // Optional if you need pricing details later
        const { pricing } = validation;
        console.log("\nCycle verified successfully.");
        ...
        console.log(err);
        webHooktryCatchErrorHandler("cycle qr scan error", err);
        return false;
    }
});

export const startScanLockerOld = asyncHandler(async (req, resp) => {
...
export const startScanLocker = asyncHandler(async (req, resp) => {
    ...
    if (!locker_available)
        return resp.json({
            status: 0,
            code: 201,
            message: ["Lockers are currently offline. Please try again shortly"],
        });
    ...
    const locker_data = await queryDB(
        ...
        ORDER BY cl.id DESC 
        LIMIT 1`,
        [cycle_id, lock_number, station_id],
    );
    if (!locker_data)
        return resp.json({
            status: 0,
            ...
            error_type: "invaild-QR",
            message: [
                "The locker QR code is incorrect. Please scan the correct QR code.",
            ],
        });

    if (locker_data.lock_number != lock_number)
        return resp.json({
            status: 0,
            code: 201,
            message: ["This cycle is not parked at given locker!"],
        });
    ...
    const area_price = await queryDB(
        ...
        FROM cycle_area_price cap
        JOIN cycle_list cl 
        ON cl.cycle_type = cap.cycle_type 
        and (
                cl.country_id = cap.country_id 
                OR cap.city_id  = cl.city_id  
            )
        WHERE cap.status = 1 
        AND cl.cycle_id = ?
        LIMIT 1`,
    ...
    //   const min_price = area_price
    //     ? area_price.min_price
    //     : pricing_details.min_price;
    const min_price = pricing_details.country_min_wallet_price;
    ...
    if (Number(pricing_details.wallet_balance) < Number(min_price)) {
        return resp.json({
            ...
            code: 201,
            error_type: "balance",
            message: ["Please recharge your wallet to start your ride."],
        });
    }
    const post_price = area_price
        ? area_price.post_price
        : pricing_details.post_price;
    const base_duration = area_price
        ? area_price.base_duration
        : pricing_details.base_duration;
    ...
    if (insert.affectedRows == 0)
        return resp.json({ status: 0, message: "Something went wrong!" });
    ...
    client.publish(
        `/supro/plusxm/slock/${locker_data.solenoid_id}/${lock_number}`,
        "ON",
        { qos: 0, retain: false },
    );
    ...
    let count = 0;
    const interval = setInterval(() => {
        client.publish(`/supro/GW/${locker_data.gateway_id}/UP`, payload, {
            qos: 0,
            retain: false,
        });
        count++;
    ...
    return resp.json({
        ...
        code: 200,
        booking_id,
        station_name: locker_data.pickup_station,
...
export const startBooking = asyncHandler(async (req, resp) => {
    ...
    await insertRecord(
        ...
        [
            ...
            "ON",
            {
                station_id: locker_data.station_id,
    ...
    await updateRecord(
        "cycle_list",
        { status: 0, lock_number: null },
        ["cycle_id"],
        [bookingData.cycle_id],
    );
    ...
    emailQueue.addEmail(
        process.env.MAIL_MOBILITY_ADMIN,
        admin_mail_template.subject({ booking_id }),
        admin_mail_template.content({
            booking_id,
...
export const stopeRideOld = asyncHandler(async (req, resp) => {
    ...
    if (oldBooking.active === 1) {
        return resp.json({
            status: 0,
            code: 422,
            message: ["Your ride has been already Ended!"],
        });
    }
    ...
    const update_rider = await db.execute(
        ...
        WHERE rider_id = ?`,
        [
            parseFloat(remaning_cost.toFixed(2)),
            parseFloat(out_standing_cost.toFixed(2)),
            rider_id,
        ],
    );
    if (!update_rider && !update_booking) {
        return resp.json({
            status: 0,
            code: 400,
            message: ["Booking was not Stopped !"],
        });
    }
    await insertRecord(
        "booking_history",
        ["booking_id", "rider_id", "status"],
        [booking_id, rider_id, "END"],
    );
    ...
    return resp.json({
        status: 1,
        code: 200,
        message: ["Booking has been stoped successfully!"],
    });
});
...
export const stopeRide = asyncHandler(async (req, resp) => {
    ...
    if (diffInMinutes > base_duration) {
        const extraMinutes = diffInMinutes - base_duration;
        total_cost = base_price + extraMinutes * post_price;
    }
    ...
    const final_amount = parseFloat((total_cost + gst).toFixed(2));

    const current_balance = Math.max(wallet_balance - final_amount, 0);

    const outstanding =
        wallet_balance >= final_amount
            ? 0
            : parseFloat((final_amount - wallet_balance).toFixed(2));
...
export const completeCycleQrOld = asyncHandler(async (req, resp) => {
    try {
        ...
        if (!oldBooking) {
            return resp.json({
                ...
                code: 201,
                message: [
                    "This QR code doesn't match our system. Please scan the correct PlusX Cycle QR code.",
                ],
            });
        }
        if (oldBooking.booked_cycle_id !== oldBooking.cycle_id) {
            return resp.json({
                status: 0,
                code: 422,
                message: ["This is not the same cycle you booked earlier!"],
            });
        }
        if (oldBooking.status === "CMP") {
            return resp.json({
                status: 0,
                code: 422,
                message: ["Your ride has been already completed!"],
            });
        }
        return resp.json({
            status: 1,
            code: 200,
            cycle_id: oldBooking.cycle_id,
            message: [`Cycle verified successfully!`],
        });
    } catch (error) {
        return resp.json({
...
export const completeCycleQr = asyncHandler(async (req, resp) => {
    try {
        ...
        const oldBooking = await queryDB(
            ...
            SELECT 
                cb.status, 
                cb.cycle_id as booked_cycle_id, 
                cl.cycle_id 
            FROM cycle_booking cb 
            JOIN cycle_list cl on cl.cycle_id=?
            WHERE  booking_id = ?`,
        ...
        // Booking not found or invalid QR
        if (!oldBooking) {
            return resp.json({
                status: 0,
                code: 404,
                ...
                message: [
                    "The booking or cycle QR code is invalid. Please scan the correct cycle QR code.",
                ],
            });
        }

        // Different cycle scanned
        if (oldBooking.booked_cycle_id !== oldBooking.cycle_id) {
            return resp.json({
                status: 0,
                code: 422,
                ...
                message: [
                    "This isn't the cycle associated with your ride. Please scan the QR code on the cycle you rented.",
                ],
            });
        }
...
export const lockerAvailable = asyncHandler(async (req, resp) => {
    ...
    const cycle_check = await queryDB(
        ...
        WHERE station_id = ? 
        and lock_number = ? `,
        [station_id, lock_number],
    );
    if (!cycle_check) {
        return resp.json({
            status: 1,
            code: 200,
            message: ["Locker is unoccupied!"],
        });
    }
    return resp.json({
        status: 0,
        code: 201,
        message: [
            "This locker is already in use. Please scan the correct locker to end your ride",
        ],
    });
});

export const completeLockerQrOld = asyncHandler(async (req, resp) => {
    const {
        rider_id,
        cycle_id,
        station_id,
    ...
    const locker_available = await queryDB(
        ...
        SELECT id 
        from cycle_locker 
        where station_id = ? and updated_at >= NOW() - INTERVAL 5 MINUTE `,
        [station_id],
    );
    if (!locker_available) {
        return resp.json({
    ...
    const bookingDetail = await queryDB(
        ...
        WHERE cb.booking_id = ? limit 1`,
        [latitude, longitude, latitude, booking_id],
    );
    if (!bookingDetail)
        return resp.json({ status: 0, code: 422, message: ["Booking not found!"] });
    ...
    const query = `
        ...
        JOIN mobility_station_list msl  
        on msl.station_id = cl.station_id
        WHERE cl.${lock_number} = 1 
        AND cl.station_id = ? 
        AND cl.updated_at >= NOW() - INTERVAL 15 MINUTE
    ...
    if (!cycle_lock) {
        return resp.json({
            status: 0,
            code: 201,
            message: ["Sorry this cycle is not locked at given locker!"],
        });
    }
  ...
  `,
    ...
    await insertRecord(
        ...
        [
            ...
            rider_id,
            "CMP",
            { station_id: station_id, lock_number, cycle_id },
        ],
    );
    ...
    if (db_logs_data?.description) {
        ...
        if (station_id !== description.station_id) {
            db.execute(
                ...
                WHERE cycle_id = ? `,
                [cycle_lock.station_id, lock_number, bookingDetail.cycle_id],
            );
        }
    }
    await updateRecord(
    ...
    await pushNotification(
        ...
        "RDRFCM",
        `Mobility_Ride_Completed/${booking_id}`,
    );
    ...
    if (outstanding > 0) {
        message = `Your ride has been completed successfully. Please pay ₹${outstanding.toFixed(
            2,
        )} to clear your pending ride payment.`;
    } else {
        message = `Your ride has been completed successfully. ₹${final_amount} has been deducted from your wallet.`;
    }
    ...
    return resp.json({
        ...
        code: 200,
        // message: [
        //   `Your ride has been completed. Please pay ₹${final_amount} to clear your ride payment`,
        // ],
        message: [message],
    ...
    try {
        ...
        if (!locker_available) {
            return resp.json({
                ...
                code: 201,
                message: [
                    "Lockers are currently offline. Please try again shortly",
                ],
            });
        }
        ...
        const bookingDetail = await queryDB(
            ...
            SELECT
                ...
                cb.status,   
                6371 * ACOS(
                    COS(RADIANS(?)) * COS(RADIANS(cb.start_lat)) *
                    COS(RADIANS(cb.start_long) - RADIANS(?)) +
                    SIN(RADIANS(?)) * SIN(RADIANS(cb.start_lat))
                ) AS distance
            FROM cycle_booking cb
            JOIN riders r 
        ...
        const query = `
            SELECT 
                ...
                msl.longitude, 
                msl.station_name AS dropoff_station, 
                msl.station_name AS station_name, 
                msl.address,
                cl.${lock_number}, 
                msl.state_id
            FROM cycle_locker cl
        ...
        if (!cycle_lock) {
            return resp.json({
                ...
                code: 201,
                message: [
                    "Sorry this cycle is not locked at given locker!",
                ],
            });
        }
        ...
        // Existing outstanding amount from previous rides
         ...
         * Wallet after ride = 0
         * Ride shortfall = 30
         * New outstanding = 20 + 30 = 50
         *
         *
         * Example 3:
         *
         * Wallet = 0
         * Existing outstanding = 40
         * Ride = 80
         *
         * Wallet after ride = 0
         * Ride shortfall = 80
         * New outstanding = 40 + 80 = 120
         */
        ...
        // Payment type remains debt as per your existing logic
        const payment_type = "debt";
        ...
        /*
         * Wallet will NEVER become negative.
         *
         * If wallet is insufficient:
         *
         * amount = 0
         * out_standing_cost = existing outstanding + shortfall
         *
         * If wallet is sufficient:
         *
         * amount = wallet - ride cost
         * out_standing_cost = existing outstanding
         */
        ...
        if (ride_outstanding > 0) {
            message = `Your ride has been completed successfully. Please pay ₹${ride_outstanding.toFixed(
                2,
            )} for this ride. Your total outstanding amount is ₹${outstanding.toFixed(
                2,
            )}.`;
        } else if (existing_outstanding > 0) {
            message = `Your ride has been completed successfully. ₹${final_amount} has been deducted from your wallet. Your total outstanding amount is ₹${outstanding.toFixed(
                2,
            )}.`;
        } else {
            message = `Your ride has been completed successfully. ₹${final_amount} has been deducted from your wallet.`;
        ...
        return resp.json({
            ...
            ride_outstanding: ride_outstanding,
            outstanding: outstanding,
        });

    } catch (error) {
        console.log(
            "completeLockerQr error:",
            error,
        );
...
export const manualRideCreateOTPOld = asyncHandler(async (req, resp) => {
    ...
    if (res.status == 0)
        return resp.json({ status: 0, code: 422, message: res.msg });

    if (!station_check)
        return resp.json({
            status: 0,
            code: 200,
            message: ["All locker is Cccupied ! "],
        });
    ...
    emailQueue.addEmail(
        station_check.operator_email,
        `Manual Handover OTP - PlusX Electric`,
        html,
    );
...
export const manualRideCreateOTP = asyncHandler(async (req, resp) => {
    try {
        ...
        let station_check = await queryDB(
            `
        SELECT operator_name, operator_contact, operator_email
        FROM mobility_station_list
        WHERE station_id = ?
      `,
        ...
        // If queryDB returns an array, get first row
        if (Array.isArray(station_check)) {
            station_check = station_check[0];
        }
        ...
        if (!station_check) {
            return resp.json({
                status: 0,
                code: 200,
                message: ["All locker is Occupied!"],
            });
        }
        ...
        const fullMobile =
            `${country_code}${station_check.operator_contact}`.replace("+", "");
      ...
      <html>
        <body>
          <h4>Hello ${station_check.operator_name},</h4>
          <p>
            Your One-Time Password (OTP) for completing the manual handover process is:
            <strong>${otp}</strong>
          </p>
          <p>
            Please use this OTP to proceed. For your security, do not share this code with anyone.
          </p>
          <p>If you did not request this, please ignore this email.</p>
          <p>
            Best regards,<br />
...
export const manualVerifyOTPOld = asyncHandler(async (req, resp) => {
    const {
        country_code = "+91",
        station_id,
        booking_id,
    ...
    if (!cachedOtp || cachedOtp !== otp)
        return resp.json({ status: 0, code: 422, message: ["OTP invalid!"] });
...
export const manualVerifyOTP = asyncHandler(async (req, resp) => {
    try {
        ...
        // Fetch Station Details
        let station_data = await queryDB(
            `
        SELECT operator_contact
        FROM mobility_station_list
        WHERE station_id = ?
      ...
      `,
        ...
        // If queryDB returns an array, get the first record
        ...
        if (!station_data) {
            return resp.json({
                ...
                code: 422,
                message: [
                    "The mobile number is not registered with us. Kindly sign up.",
                ],
            });
        }
        ...
        // Prepare Mobile Number
        const fullMobile =
            `${country_code}${station_data.operator_contact}`.replace("+", "");
        ...
        console.error("Error in manualVerifyOTP:", err);
    ...
    try {
        const bookingDetail = await queryDB(
            ...
            SELECT
                ...
                cb.rider_id, 
                r.amount as wallet_balance,
                r.out_standing_cost as previous_outstanding,
                cb.status, 
                cb.price,
                msl.state_id
            FROM cycle_booking cb
        ...
        if (bookingDetail.status === "CMP")
            return {
                ...
                code: 422,
                message: [
                    "Your ride has been already completed!",
                ],
            };
        ...
        const previous_outstanding = parseFloat(
            bookingDetail.previous_outstanding || 0,
        );
        ...
        /*
         * Keep your existing GST logic here.
         *
         * NOTE:
         * Your pasted code uses cycle_lock.state_id,
         * but cycle_lock is not defined in this function.
         * I am using bookingDetail.state_id because it is
         * already available from your query.
         */
        const gst =
        ...
        const finalAmountNum =
            parseFloat(final_amount);

        /*
         * CASE 1:
         * Wallet is enough to pay this ride.
         *
         * Example:
         * wallet = 500
         * ride = 200
        ...
        if (wallet_balance >= finalAmountNum) {
            var current_balance = parseFloat(
                (
                    wallet_balance -
                    finalAmountNum
                ).toFixed(2),
         ...
         * previous outstanding = 50
         *
         * ride outstanding = 200
         * new outstanding = 50 + 200 = 250
         * wallet = 0
         */
        else {
            const rideOutstanding =
                parseFloat(
                    (
                        finalAmountNum -
                        wallet_balance
        ...
        if (
            station_id !==
            db_logs_data.description.station_id
        ) {
            const updtObj = {
                station_id:
                    drop_station.station_id,
                lock_number: lock_number,
            };
...
// 2 Apr
export const startBookingCheck = async (
    rider_id,
    booking_id,
    station_id,
    lock_number,
    ...
    try {
        ...
        if (locker_data[lock_number] == 0) {
            ...
            const admin_mail_template =
                NOTIFICATION_CONTENT["ADMIN_RIDE_START_EMAIL"];
            ...
            await sendNotification(
                ...
                rider_id,
                "",
            ); //Incomplete Booking – Booking ID: XXXX
            io.emit("notification-list", { msCount: 1 });
...
export const havingIssueBooking = asyncHandler(async (req, resp) => {
    ...
    await insertRecord(
        ...
        [
            ...
            bookingData.city,
            bookingData.lock_number,
            bookingData.university,
            bookingData.start_lat,
            bookingData.start_long,
            type_of_issue,
        ],
    );
    return resp.json({
...
export const feedbackBooking = asyncHandler(async (req, resp) => {
    ...
    if (!bookingData)
        return resp.json({
            ...
            code: 201,
            message: [
                "Feedback submission is not allowed. The booking is not completed yet.",
            ],
        });

    await insertRecord(
        "cycle_booking_feedback",
...
export const stationLockerUpdate = asyncHandler(async (req, resp) => {
    ...
    const message =
        operation == 1 ? "Cycle out from locker" : `Cycle parked at locker`;
...
export const stationLockerRandomDataUpdate = asyncHandler(async (req, resp) => {
    const { station_id } = mergeParam(req);
    ...
    const currentValue = parseInt(locker[0].random_data ?? -1, 10);

    const MAX_LIMIT = 99;

    const nextValue = currentValue >= MAX_LIMIT ? -1 : currentValue + 1;

    await db.execute(
        `UPDATE cycle_locker
     SET random_data = ?
     WHERE station_id = ?`,
        [nextValue, station_id],
    );

    return resp.json({
        status: 1,
        code: 200,
        random_data: nextValue,
        message: ["Random data updated successfully"],
    });
});

export const requestRefundOld = asyncHandler(async (req, resp) => {
    try {
        ...
        SELECT 
          rider_id, 
          CONCAT(rider_name, ' ', last_name) AS user_name, 
          rider_mobile, 
          amount, 
          out_standing_cost 
        FROM riders 
        ...
        const pendingRequest = await queryDB(
            `
        SELECT id FROM refund_requests WHERE rider_id = ? AND status IN ('pending') LIMIT 1`,
            [rider_id],
        );
        ...
        const deductionAmount = Number(((walletAmount * 2) / 100).toFixed(2));

        const refundAmount = Number((walletAmount - deductionAmount).toFixed(2));
...
export const requestRefund = asyncHandler(async (req, resp) => {
    try {
      ...
      `,
        ...
        // Deduct outstanding amount first
        const refundableAmount = Number(
            (securityDeposit - outstandingAmount).toFixed(2),
        );
        ...
        // 3% processing charge
        const processingFee = Number((refundableAmount * 0.03).toFixed(2));

        const finalRefundAmount = Number(
            (refundableAmount - processingFee).toFixed(2),
        );
        ...
        return resp.json({
            status: 1,
            code: 200,
            message: ["We have received your security deposit refund request."],
            data: {
