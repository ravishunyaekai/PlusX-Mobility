...
export const chargerBooking = asyncHandler(async (req, resp) => {
    ...
    const {
        rider_id, user_name, country_code, contact_no, address, latitude, longitude, parking_number = '', parking_floor = '', vehicle_id, slot_date, slot_time, slot_id, service_price = 0, address_id, device_name = '', coupon_code = '', battery_percent = 0, package_id
    } = mergeParam(req);
    ...
    try {
        ...
        if (parseFloat(service_price).toFixed(2) != parseFloat(bookingPrice).toFixed(2) && coupon_code == '') {
            ...
            if (couponData.status == 0) {
                ...
                return resp.json({ 
                    status: 0, code: 422, error: true, message: ["Booking price is not valid!"], expected_price: expectedPrice
                });
            }
        }
        ...
        if (fSlotDateTime < currDateTime) return resp.json({
            status: 0, code: 422, message: ["Invalid slot, Please select another slot"]
        });
...
export const reScheduleBooking = asyncHandler(async (req, resp) => {
    ...
    try {
        ...
        emailQueue.addEmail(process.env.MAIL_POD_ADMIN, `PlusX Electric- Mobile EV Charging Booking Rescheduled (Booking ID : ${booking_id} )`, htmlAdmin);
