...
export const evInsuranceDetail = asyncHandler(async (req, resp) => {
    ...
    const data = await queryDB(`
        SELECT 
           insurance_id,rider_id, owner_name, country_code, mobile_no, notionality, date_of_birth, ev_type,vehicle_data, vehicle, chassis_number,
  ...
  const imageFields = [
    "driving_licence",
    "prev_insurance_image",
    "claim_letter_image"
  ];
 imageFields.forEach(field => {
    const images = parseImages(data[field]);

    images.forEach(img => {
      gallery_data.push(img);
      gallery_id.push(galleryAutoId++);
    });
    ...
    data.base_url     = `${process.env.DIR_UPLOADS}insurance-images/`;
...
const parseImages = (value) => { if (!value) return []; return value .split("*") .map(v => v.trim()) .filter(Boolean); };
