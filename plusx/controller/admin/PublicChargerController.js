...
import fs from 'fs';
import { getOpenAndCloseTimings, formatOpenAndCloseTimings, deleteFile, asyncHandler, formatDateTimeInQuery, formatAllTimeTimings} from '../../../utils.js';
...
export const stationData = asyncHandler(async (req, resp) => {
    ...
    const chargingFor =['All EV`s','Tata', 'MG', 'Hyundai', 'Mahindra', 'BYD'];
...
export const stationDetail = asyncHandler(async (req, resp) => {
    ...
    [gallery] = await db.execute(`
        SELECT 
            id, image_name 
        FROM 
            public_charging_station_gallery 
        WHERE 
            station_id = ? 
        ORDER BY id DESC `, 
    [station_id]);
...
// updated as per paramjeet
export const oldaddPublicCharger = asyncHandler(async (req, resp) => {
    try {
    ...
    const stationId = `TRQ${generateUniqueId({ length:6 })}`;  
        ...
        [
            'station_id', 'station_name', 'price', 'description', 'charging_for',
             'charger_type', 'charging_point', 'address', 'latitude', 'longitude', 
             'station_image', 'status', 'always_open', 'open_timing',
            'available_charging_point', 'occupied_charging_point'  
        ], [
            stationId, station_name, price, description, charging_for, charger_type, 
            charging_point, address, latitude, longitude, stationImg, 1, always_open,fTiming, availableChargingPoint,
...
export const addPublicCharger = asyncHandler(async (req, resp) => {
    try {
        ...
        const { fDays, fTiming } = formatOpenAndCloseTimings(always_open, data);
        const { all_days, all_time } = formatAllTimeTimings(always_open, data);
    ...
    const stationId = `TRQ${generateUniqueId({ length:6 })}`;  
        ...
        [
            ...
            'available_charging_point', 'occupied_charging_point','all_time'  
        ], [
            ...
            occupiedChargingPoint,all_time
        ]);
