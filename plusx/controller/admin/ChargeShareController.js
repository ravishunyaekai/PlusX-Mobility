...
export const acceptChargShare = async (req, resp) => {
    try {
        ...
        return resp.json({ status: 1, code: 200, message: "Request accepted successfully" });
...
export const outputAndConnector = asyncHandler(async (req, resp) => {
    ...
    switch (requirement) {
        ...
        SELECT value
        FROM output_connector
        WHERE status = 'ac-output'
        ORDER BY CAST(
