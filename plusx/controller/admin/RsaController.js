...
export const rsaAdd = asyncHandler(async (req, resp) => {
    const { rsa_name, rsa_email, mobile, service_type, password, confirm_password, country_code = "+91" } = req.body;
    const { isValid, errors } = validateFields(req.body, {
        rsa_name: ["required"],
        rsa_email: ["required"],
    ...
    const insert = await insertRecord('rsa', [
        ...
        `Driver-${generateUniqueId({ length: 5 })}`, rsa_name, rsa_email, country_code, mobile, service_type, hashedPswd, 0, 0, profile_image
    ]);
...
export const rsaUpdate = asyncHandler(async (req, resp) => {
    const { rsa_id, rsa_name, rsa_email, mobile, service_type, password, confirm_password, country_code = "+91" } = req.body;
    const { isValid, errors } = validateFields(req.body, {
        rsa_id: ["required"],
        rsa_name: ["required"],
        rsa_email: ["required"],
    ...
    const updates = { rsa_name, email: rsa_email, mobile, country_code, booking_type: service_type, profile_img: profile_image };
