...
const chargerBooking = async (req, resp) => {
    ...
    if( insert.affectedRows > 0 ) {
        ...
        const heading_admin = 'EV Charger Booking';
        const desc_admin    = `Booking ID: ${requestId}`;
        createNotification(heading_admin, desc_admin, 'EV Charger Booking', 'Admin', 'Rider', rider_id, '', href);
        io.emit('plusx-notification-list', {msCount : 1});
        const heading_user=`PlusX ${chargerName}`;
        const desc_user=`Booking ID: ${requestId}`;

        createNotification(heading_user, desc_user, 'EV Charger Booking', 'Rider', 'Admin', '', rider_id, href);
      
        // const desc1    = `PlusX ${chargerName} - Booking ID : ${requestId}`;
      
        pushNotification(rider.fcm_token, heading_user, desc_user, 'RDRFCM', href);
...
const accessoriesBooking = async (req, resp) => {
    ...
    if( insert.affectedRows > 0 ) {
        ...
        const desc    = `Booking ID: ${requestId}`;
        io.emit('plusx-notification-list', {msCount : 1});
        ...
        const heading_user=`PlusX ${chargerName}`;
        const desc_user=`Booking ID: ${requestId}`;
        createNotification(heading_user, desc_user, 'EV Accessories Booking', 'Rider', 'Admin', '', rider_id, href);
       // const desc1    = `PlusX ${chargerName} - Booking ID : ${requestId}`;
        pushNotification(rider.fcm_token,heading_user, desc_user, 'RDRFCM', href);
...
const installationBooking = async (req, resp) => {
    ...
    if( insert.affectedRows > 0 ) {
        ...
        const heading_admin = 'EV Charger Booking ';
        const desc_admin    = `Booking ID : ${requestId}`;
        //PlusX EV Charger Installation  Booking ID: [Booking ID]
        createNotification(heading_admin, desc_admin, 'Charging Installation Service', 'Admin', 'Rider', rider_id, '', href);
        const heading_user=`EV Charger Installation Booking`;
        const desc_user=`Booking ID: ${requestId}`;

        createNotification(heading_user, desc_user, 'Charging Installation Service', 'Rider', 'Admin', '', rider_id, href);
         io.emit('plusx-notification-list', {msCount : 1});
        //EV Charger Installation Booking – [Booking ID]
...
export const evchargerDetails = asyncHandler(async (req, resp) => { 
    ...
    const compatible       =  chargerData.compatible.map(item => item.label) ||null;
