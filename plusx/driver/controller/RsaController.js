    ...
    if (bookingType === "Portable Charger") {
            ...
            LEFT JOIN (
                SELECT 
                    ...
                    COUNT(
                        CASE 
                            WHEN pba.status = 1 
                            AND pcb.status NOT IN ('PU', 'CC', 'RO', 'C')
                            THEN 1 
                        END
                    ) AS running,
                ...
                FROM portable_charger_booking_assign pba

                INNER JOIN portable_charger_booking pcb
                    ON pcb.booking_id = pba.order_id
        ...
        const [podAssign] = await db.execute(`
            SELECT 
                ...
                pb.booking_id as request_id, 
                pb.address as pickup_address, 
                pb.latitude as pickup_latitude, 
                pb.longitude pickup_longitude, 
                ...
                (
                    SELECT CONCAT(vehicle_make, "-", vehicle_model) 
                    FROM riders_vehicles 
                    WHERE vehicle_id = pb.vehicle_id
                ) AS vehicle_data,
                COALESCE(
                    (
                        SELECT guideline 
                        FROM portable_charger_history pch 
                        WHERE pch.rider_id = pb.rider_id 
                        AND pch.order_status = 'CS' 
                        LIMIT 1
                    ),''
                ) AS guideline,
            ...
            LEFT JOIN portable_charger_booking AS pb 
            ON pb.booking_id = portable_charger_booking_assign.order_id
        ...
        const COMPLETED_STATUSES = ['PU', 'CC', 'RO'];
        const CLOSED_STATUSES = ['PU', 'CC', 'RO', 'C'];

        result.assigned_booking = podAssign.filter(
            item =>
                item.assign_status === 0 &&
                !CLOSED_STATUSES.includes(item.order_status)
        );

        result.ongoing_booking = podAssign.filter(
            item =>
...
export const rsaHome2 = asyncHandler(async (req, resp) => {
    const { rsa_id } = mergeParam(req);
...
export const rsaHome1 = asyncHandler(async (req, resp) => {
    const { rsa_id } = mergeParam(req);
    console.log("rsa_id", rsa_id)
    const rsaData = await queryDB(`
        SELECT 
            r.status, 
            r.booking_type,
...
export const rsaBookingHistory = asyncHandler(async (req, resp) => {
    ...
    if (booking_type === 'PCB') {
        ...
        const [podCompleted] = await db.execute(`
            ...
            LEFT JOIN portable_charger_history AS pch 
                ON CONVERT(pcb.booking_id USING utf8mb4)
                    COLLATE utf8mb4_unicode_ci
                =
                   CONVERT(pch.booking_id USING utf8mb4)
                    COLLATE utf8mb4_unicode_ci
    ...
    // ============================================================
        ...
        const [rsaCompleted] = await db.execute(`
            ...
            FROM road_assistance AS rsa

            LEFT JOIN order_history AS rsah 
                ON CONVERT(rsa.request_id USING utf8mb4)
                    COLLATE utf8mb4_unicode_ci
                =
                   CONVERT(rsah.order_id USING utf8mb4)
                    COLLATE utf8mb4_unicode_ci

            WHERE 
                rsa.rsa_id = ?
                AND rsa.order_status = 'RO'

            GROUP BY rsa.request_id

            ORDER BY rsa.updated_at DESC
...
export const rsaBookingHistoryOld1 = asyncHandler(async (req, resp) => {
    const { rsa_id, booking_type } = mergeParam(req);
    const { isValid, errors } = validateFields(mergeParam(req), {
        rsa_id: ["required"],
        booking_type: ["required"]
