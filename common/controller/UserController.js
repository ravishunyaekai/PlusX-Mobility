...
export const notificationList = asyncHandler(async (req, resp) => {
  ...
  const totalRows = await queryDB(
    `SELECT COUNT(*) AS total FROM notifications WHERE   panel_to = ? AND receive_id = ?`,
    ["Rider", rider_id],
  );
  const total_page = Math.ceil(totalRows.total / limit) || 1;

  const [rows] = await db.execute(
    `SELECT id, heading, description, module_name, panel_to, panel_from, receive_id, status, ${formatDateTimeInQuery(["created_at"])}, href_url
  ...
  await db.execute(
    `UPDATE notifications SET status=? WHERE  status=? AND panel_to=? AND receive_id=?`,
    ["1", "0", "Rider", rider_id],
  );
...
export const forgotPassword = asyncHandler(async (req, resp) => {
  ...
  const [[rider]] = await db.execute(
    "SELECT rider_name FROM riders WHERE rider_email=?",
    [email],
  );
  ...
  await db.execute("UPDATE riders SET password=? WHERE rider_email=?", [
    hashedPswd,
    email,
  ]);
  ...
  try {
    ...
    emailQueue.addEmail(
      email,
      `Forgot Password Request - PlusX Electric App`,
      html,
    );
    ...
    return resp.status(200).json({
      ...
      code: 200,
      message:
        "Password Reset Request! We have sent the new password to your registered email.",
    });
  } catch (error) {
    console.log(error);
...
export const logout = asyncHandler(async (req, resp) => {
  ...
  const rider = queryDB(
    `SELECT EXISTS (SELECT 1 FROM riders WHERE rider_id = ?) AS rider_exists`,
    [rider_id],
  );
  if (!rider)
    return resp.json({ status: 0, code: 400, message: "Rider ID Invalid!" });

  const update = await updateRecord(
    "riders",
    { status: 0, access_token: "", fcm_token: "" },
    ["rider_id"],
    [rider_id],
  );
...
export const updatePassword = asyncHandler(async (req, resp) => {
  const { rider_id, old_password, new_password, confirm_password } =
    mergeParam(req);
  ...
  const rider = await queryDB(`SELECT password FROM riders WHERE rider_id=?`, [
    rider_id,
  ]);
  ...
  return resp.json({
    ...
    message:
      update.affectedRows > 0
        ...
        : ["Failed to updated password. Please Try Again"],
  });
});
...
export const updateProfile = asyncHandler(async (req, resp) => {
  ...
  const rider = await queryDB(
    `SELECT state_id ,city_id ,last_name,rider_name,rider_email ,profile_img FROM riders WHERE rider_id=?`,
    [rider_id],
  );
  const other_rider = await queryDB(
    `SELECT id FROM riders WHERE rider_email=? and rider_id !=?`,
    [rider_email, rider_id],
  );
  if (other_rider) {
    return resp.json({
      status: 0,
  ...
  rider.rider_email !== rider_email
    ? (updates.rider_email = rider_email)
    : null;
  ...
  if (state_id && state_id !== rider.state_id) {
    ...
    const state = await queryDB(`SELECT name FROM states WHERE state_id=?`, [
      state_id,
    ]);
  ...
  if (insert.affectedRows == 0) {
    return resp.json({
      status: 0,
      code: 400,
      message: ["Rider profile was not updated!"],
    });
  }
...
export const deleteImg = asyncHandler(async (req, resp) => {
  ...
  const rider = await queryDB(
    `SELECT profile_img FROM riders WHERE rider_id = ?`,
    [rider_id],
  );
  if (!rider)
    return resp.json({ status: 0, code: 400, message: "Rider ID Invalid!" });
  ...
  if (insert.affectedRows == 0) {
    return resp.json({
      status: 0,
      code: 400,
      message: ["Rider profile image was not deleted!"],
    });
  }
  const update = await updateRecord(
    "riders",
  ...
  return resp.json({
    ...
    message:
      update.affectedRows > 0
        ? ["Rider profile image deleted successfully!"]
  ...
  try {
    ...
    if (totalDeleted === 0)
      return resp.json({
        ...
        code: 200,
        error: false,
        message: ["Rider Account was not deleted !"],
      });
  ...
  switch (requirement) {
    case "country":
      [list] = await db.execute(
        `SELECT country_id ,name , null as state_id, null as city_id FROM country  ORDER BY name ASC`,
      );
    ...
    case "state":
      if (!country_id) {
        return resp.json({
          status: 0,
          code: 422,
          message: ["country_id is required for state list"],
        });
      }
      [list] = await db.execute(
        `SELECT  country_id, state_id,  name, null as city_id  FROM states WHERE country_id = ? ORDER BY name ASC`,
        [country_id],
      );
      return resp.json({
        status: 1,
    ...
    case "city":
      ...
      [list] = await db.execute(
        `SELECT city_id,name FROM cities where state_id=? ORDER BY name ASC`,
        [state_id],
      );
      return resp.json({
        status: 1,
        code: 200,
        data: list,
        message: ["cities List fetch successfully!"],
      });
...
//             riders
//         LIMIT 1 `,
//     [rider_email, mobile], //, fullMobile
//   );
//   if (isExist.check_mob > 0)
//     return resp.json({
//       status: 0,
//       code: 422,
//       message: ["The provided number already exists."],
//     }); // || isExist.rsa_mob > 0
...
//   emailQueue.addEmail(
//     rider_email,
//     `Your OTP for Signup - PlusX Electric`,
//     html,
//   );
...
export const register = asyncHandler(async (req, resp) => {
  ...
  const rider = await insertRecord(
    "riders",
    [
      "rider_id",
      "rider_name",
  ...
  await createNotification(
    `New User Signup`,
    `${first_name} ${last_name} signed up`,
    `mobility`,
    "Admin",
    "user",
    riderId,
    "",
    `app-signup/app-signup-details/${riderId}`,
  );
...
export const login = asyncHandler(async (req, resp) => {
  ...
  const [update] = await db.execute(
    `UPDATE riders SET access_token = ?, status = ?, fcm_token = ? WHERE rider_mobile = ?`,
    [token, 1, fcm_token, mobile],
  );
  if (update.affectedRows > 0) {
    const result = {
      image_url: `${process.env.DIR_UPLOADS}rider_profile/`,
...
export const createOTP = asyncHandler(async (req, resp) => {
  ...
  if (!riderData)
    return resp.json({
      status: 0,
      code: 422,
      message: ["The provided mobile number is not registered."],
    });

  const fullMobile = `${country_code}${mobile}`.replace("+", "");
  let otp =
    mobile == "9459459459" || mobile == "9410934120" || mobile == "9879879879"
      ? "9404"
      : generateOTP(4);
  storeOTP(fullMobile, otp);
  ...
  emailQueue.addEmail(
    ...
    `Your OTP for Signin - PlusX Electric`,
    html,
  );
...
export const verifyOTP = asyncHandler(async (req, resp) => {
  ...
  const riderData = await queryDB(
    `SELECT    customer_id ,rider_id, rider_name, last_name, rider_email, profile_img, state, status FROM riders WHERE rider_mobile =? AND country_code =? LIMIT 1`,
    [mobile, country_code],
  );
  ...
  await updateRecord(
    "riders",
    { access_token: token, status: 1, fcm_token, device_name },
    ["rider_mobile", "country_code"],
    [mobile, country_code],
  );
...
export const countryList = asyncHandler(async (req, resp) => {
  const [list] = await db.execute(
    `SELECT name, iso_code, dial_code FROM country ORDER BY name ASC`,
  );
  return resp.json({
    status: 1,
    code: 200,
...
export const redeemCoupon = asyncHandler(async (req, resp) => {
  ...
  const [[{ count }]] = await db.execute(
    "SELECT COUNT(*) AS count FROM coupon WHERE coupan_code = ?",
    [coupon_code],
  );
  if (count === 0)
    return resp.json({
  ...
  if (moment(coupon.end_date).isBefore(moment(), "day") || coupon.status < 1) {
    ...
    return resp.json({
      ...
      code: 422,
      message: [
        "This coupon code has already been used the maximum number of times.",
      ],
    });
  }
  const data = {};
  ...
  return resp.json({
    ...
    message: [
      "Your discount has been successfully applied. Enjoy the savings!",
    ],
    status: 1,
    code: 200,
  });
